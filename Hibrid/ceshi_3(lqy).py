...
class HybridSN(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out.view(-1, out.shape[1] * out.shape[2], out.shape[3],
                       out.shape[4])  # 进行二维卷积，因此把前面的 32*18 reshape 一下，得到 （576, 19, 19）
...
def train(train_loader, epochs):
    ...
    for epoch in range(epochs):#
        ...
        if ((epoch + 1) % 10 == 0 and (epoch + 1) >= 95):
...
if __name__ == '__main__':
    ...
    print('Training_Time: ', Training_Time,'Test_time: ',Test_time)
    print('y_pred_test_size: ',y_pred_test.size)
    print('y_test_size: ',y_test.size)
    print('oa: ', oa), print('aa: ', aa), print('kappa: ', kappa)
